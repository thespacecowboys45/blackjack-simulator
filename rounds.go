...
type Round struct {
	...
	// The player's hand.
	//Player Hand // @todo - remove sigle player single hand code

	// Implement multiple players sitting for a round	
	num_players int
	
	Players []Hand     // @todo - remove, single-hand code
	//Outcomes []Outcome // @todo - remove, single-hand code
...
func (round Round) toString() {
	...
	/*
	for j:=0; j<round.num_players; j++ {
		dlog.Always("round.Outcome #%d: %s", j, outcomeToString(round.Outcomes[j]))
	}
	*/
}
...
Nov '23 -  maybe I need to seed the random number generator (again?) (see func seedRand())
...
// ///////////////////////////////////////////
//
// @todo - deprecate single hand code
//
...
func (round *Round) dealToMultiPlayer(player_num int) {
	...
	// 11/26 - break single-hand code, make only multi-hand code function
	// 11/27 - deprecate single hand code
...
func (round *Round) PlayMultiPlayer(determineAction func(round Round, player_number int, hand_number int) Action, allowsplits bool) int {
	log.Printf("[rounds.go][PlayMultiPlayer][entry]")
	
	// Total number of hands played this round
	total_hands_played_this_round := 0
	dlog.Debug("[rounds.go][PlayMultiPlayer][initialize total_hands_played_this_round to %d][allowsplits %t]", total_hands_played_this_round, allowsplits)
	...
	if len(round.deck) < MINIMUM_SHOE_SIZE {
		dlog.Error("[rounds.go] HOW DID WE GET HERE?  New round, with not enough cards in the deck?????")
		// multi-player code
		...
		return total_hands_played_this_round
	}
...
//			if round.Players[i].CanSplit() {
			if round.PlayersObj[i].Hands[activeHand].CanSplit(allowsplits) {
...
//				h1, h2 := round.Players[i].Split()
				...
				round.PlayersObj[i].splitsPlayed += 1
...
//				for i:=0; i<round.num_players; i++ {
				...
				dlog.Info("[rounds.go][PlayMultiPlayer()][ran out of cards on player #%d hand %d]", i, round.PlayersObj[i].activeHand)
				
				round.PlayersObj[i].Outcomes[round.PlayersObj[i].activeHand] = OUTCOME_ABORT

/*
// Take this out, I do not think this logic is correct
...
*/		
	...
	if everyoneBusted {
	 ...
	 dlog.Always("[rounds.go][PlayMultiPlayer()][everyone did not bust.  Dealer's turn, plays: %s]", round.Dealer)
	...
	for i:=0; i<num_players; i++ {
		...
		for j:=0; j<len(round.PlayersObj[i].Hands); j++ {
			dlog.Info("Compare dealer %s [%d] to player %d hand %d %s [%d]", round.Dealer, round.Dealer.Sum(), i, j, round.PlayersObj[i].Hands[j], round.PlayersObj[i].Hands[j].Sum())
			if round.PlayersObj[i].Hands[j].IsBusted() {
				if verbose {
					...
					dlog.Info("[rounds.go][PlayMultiPlayer()][player #%d hand #%d] Player busted this hand. Dealer: %s, Player: %s", 
						i, 
						j,
						round.Dealer, 
			...
			//} else if round.Players[i].Sum() == round.Dealer.Sum() {
				...
				if verbose {
					...
					dlog.Always("[rounds.go][PlayMultiPlayer()] Player #%d hand #%d wins! Dealer: %s, Player hand: %s", 
						i, 
						j, 
						round.Dealer,
						round.PlayersObj[i].Hands[j])
			...
			dlog.Error("[rounds.go] HOW DID WE GET HERE?")
			log.Fatal("Something is wrong with player/dealer hand comparison.  Fix this.")
		}
...
/*
 dxb - don't use time to seed (too unpredictable!)  hahaha
...
*/
		...
		round.PlayersObj[j].splitsPlayed = 0
